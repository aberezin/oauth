(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('oauthdbApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-oauthdbApp-error');
                var entityKey = httpResponse.headers('X-oauthdbApp-params');
                if (errorHeader) {
